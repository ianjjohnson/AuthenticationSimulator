...
ts.plot(times$X4, xlab="Message Number", ylab="Jitter (MS)", main="Timeseries of CPU Delay")
@
...
\begin{center}
<<echo=FALSE, fig=TRUE>>=
hist(times$X4+0.5,freq = FALSE, breaks=-1:(max(times$X4)+1)+0.5, xlab="Time passed expected (ms)", main = "Histogram of CPU Delay")
...
plot(auth$window, auth$tp / (auth$tp + auth$fn), main = "Positive and Negative Authentication Accuracies", ylab="Score", xlab="Window Size")
...
plot(auth$window, auth$fp + 0.5*auth$fn, main = "Cost Scores", ylab="Score", xlab="Window Size")
lines(auth$window, auth$fp + 0.5*auth$fn, col= "Blue")
@
